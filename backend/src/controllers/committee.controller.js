...
exports.saveScore = catchAsync(async (req, res, next) => {
  ...
  const [sig] = await query('SELECT id FROM signatures WHERE assignment_id = ? AND is_active = 1', [assignment.id]);
...
exports.sign = catchAsync(async (req, res, next) => {
  ...
  const [active] = await query('SELECT id FROM signatures WHERE assignment_id = ? AND is_active = 1', [assignment.id]);
